...
interface CategoryProps extends ICategoryData {
    favorites?: string[]; // Array of favorited route_ids
...
const Category: React.FC<CategoryProps> = ({
    ...
    const handleStarClick = (e: React.MouseEvent, routeId: string) => {
        e.stopPropagation();
        if (onToggleFavorite) {
    ...
    return (
        <div className='flex flex-col w-full px-10 py-5'>
            ...
            {routes.map(route => {
                ...
                return (
                    <div key={route.id} className="flex items-center gap-2 group">
                        <button
                        ...
                        {showStars && onToggleFavorite && (
                            <button
                                onClick={(e) => handleStarClick(e, route.id)}
